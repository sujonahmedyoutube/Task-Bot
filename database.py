...
class Database:
        ...
        try:
            if not firebase_admin._apps:
                ...
                if os.path.exists(cred_path):
                    ...
                    print("⚠️ ERROR: firebase-credentials.json not found!")
    ...
    def initialize_collections(self):
        """Initialize all required collections with sample data if empty"""
        try:
    ...
    async def get_user(self, user_id: int) -> Optional[Dict]:
        ...
        try:
            user_data = self.db.child('users').child(str(user_id)).get()
            return user_data
        except Exception as e:
    ...
    async def remove_task(self, task_id: str) -> bool:
        """Remove a task (set status to inactive)"""
        try:
    ...
    async def approve_submission(self, submission_id: str) -> bool:
        """Approve a submission and add reward to user"""
        try:
    ...
    async def reject_submission(self, submission_id: str, reason: str) -> bool:
        ...
        try:
            ...
            print(f"Error rejecting submission {submission_id}: {e}")
            return False
    ...
    async def process_withdrawal(self, withdrawal_id: str, status: str, reason: str = None) -> bool:
        ...
        try:
            ...
            print(f"Error processing withdrawal {withdrawal_id}: {e}")
            return False
    ...
    async def redeem_gift_code(self, code: str, user_id: int) -> Dict:
        ...
        try:
            ...
            if user_id in used_users and usage_limit != -1:
                return {'success': False, 'message': 'You have already used this gift code'}
            ...
            return {'success': True, 'message': f'Successfully redeemed {code_data["amount"]} coins!'}
        except Exception as e:
    ...
    async def create_gift_code(self, code: str, amount: float, usage_limit: int, expiry_days: int = None) -> bool:
        ...
        try:
            ...
            if expiry_days:
                ...
                code_data['expiry_date'] = expiry_date.isoformat()
    ...
    async def add_force_join_channel(self, channel_id: str) -> bool:
        """Add a channel that users must join"""
        try:
            channel_data = {
                'channel_id': channel_id,
                'created_at': datetime.now().isoformat()
            }
            self.db.child('channels').child(channel_id).set(channel_data)
            return True
        except Exception as e:
